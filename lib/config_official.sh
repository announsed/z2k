...
generate_nfqws2_opt_from_strategies() {
    ...
    # Circular params match TLS profiles (fails/retrans/maxseq/reset/nld).
    # Strategy 1 (z4r): fake(0x0E0E0F0E, tcp_md5) + multisplit(host+1, seqovl=2)
    # Strategy 2 (remittor): fake(http_iana, ip_autottl=2, badsum) + fakedsplit(host+1)
    add_hostlist_line "${extra_strats_dir}/TCP/RKN/List.txt" "--filter-tcp=80 --payload=http_req --hostlist-exclude=${lists_dir}/whitelist.txt --hostlist=${extra_strats_dir}/TCP/RKN/List.txt --lua-desync=circular:fails=3:retrans=2:maxseq=16384:time=60:reset:key=http_rkn:nld=2 --lua-desync=fake:payload=http_req:dir=out:blob=0x0E0E0F0E:tcp_md5:strategy=1 --lua-desync=multisplit:payload=http_req:dir=out:pos=host+1:seqovl=2:strategy=1 --lua-desync=fake:payload=http_req:dir=out:blob=http_iana:ip_autottl=2:badsum:strategy=2 --lua-desync=fakedsplit:payload=http_req:dir=out:pos=host+1:strategy=2 --new"
